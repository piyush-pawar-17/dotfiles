...
NERD_FONT = "GeistMono Nerd Font, Geist Mono, monospace"
SCOPES = [
        ...
        for cal_id in calendar_ids:
            ...
            for item in result.get('items', []):
                ...
                if 'dateTime' in start:
                    # Timed event: retain the time and Google Calendar URL for the UI.
                    ...
                    events_by_date.setdefault(dt.date().isoformat(), []).append({
                        'type': 'event',
                        'title': title,
                        'time': f"{dt.strftime('%H:%M')}-{dt_end.strftime('%H:%M')}",
                    ...
                    while d < d_end:
                        events_by_date.setdefault(d.isoformat(), []).append(event)
                        d += datetime.timedelta(days=1)

        # The Birthday calendar is not exposed through Calendar API for every account.
        # Read contact birthdays directly through Google People instead.
        try:
            ...
            while request is not None:
                ...
                for person in result.get('connections', []):
                    name = next(
                        (entry.get('displayName') for entry in person.get('names', [])
                ...
                request = people_service.people().connections().list_next(request, result)
        except Exception as e:
            print(f'[birthday sync error] {e}', file=sys.stderr)

        # A disabled Tasks API must not discard successfully fetched calendar events.
        try:
            ...
            print(f'[tasks sync error] {e}', file=sys.stderr)
...
class CalendarPopup(Gtk.ApplicationWindow):
    ...
    def _build_grid(self):
        ...
        for i in range(total_days):
            ...
            has_events = any(
                not (isinstance(e, dict) and e.get('type') == 'task')
                for e in day_events
            )
            has_tasks = any(
                isinstance(e, dict) and e.get('type') == 'task' and not e.get('done')
                for e in day_events
            )
    ...
    def _make_event_row(self, ev):
        ...
        if isinstance(ev, dict) and ev.get('type') == 'task':
            ...
            # Keep the event details together on the left and reserve the right edge for its link.
            event = ev if isinstance(ev, dict) else {'title': ev, 'time': '', 'url': ''}
            details = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            details.set_hexpand(True)
            time = Gtk.Label(label=event.get('time', ''))
            ...
            if event.get('url'):
                open_btn = Gtk.Button()
                open_btn.add_css_class('event-open')
                open_btn.set_tooltip_text('Open in Google Calendar')
                open_btn.set_size_request(22, 22)
                open_lbl = Gtk.Label(label="󰏌")
                open_lbl.add_css_class('nerd')
                open_btn.set_child(open_lbl)
                open_btn.connect('clicked', self._on_event_open, event['url'])
                row.append(open_btn)
    ...
    def _update_day_panel(self, date):
        ...
        day_events = sorted(
            ...
            key=lambda e: 0 if (isinstance(e, dict) and e.get('type') == 'task' and not e.get('done')) else (
                1 if (isinstance(e, dict) and e.get('type') == 'task') else 2
            ),
        )
        if day_events:
